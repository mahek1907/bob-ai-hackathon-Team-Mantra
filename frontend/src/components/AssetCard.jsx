...
export default function AssetCard({ asset, onGenerateWorkOrder, isGenerating }) {
  ...
  const getDgaGasStatus = (gas, ppm) => {
    ...
    if (ppm > limit * 3) return { label: 'CRITICAL', color: 'bg-red-50 text-red-700 font-bold border border-red-200' };
    if (ppm > limit) return { label: 'ELEVATED', color: 'bg-amber-50 text-amber-800 font-semibold border border-amber-200' };
    return { label: 'NORMAL', color: 'bg-emerald-50 text-emerald-800 border border-emerald-200' };
  };
  ...
  return (
    <div className={`rounded-xl border transition-colors bg-white ${
      asset.risk_category === 'CRITICAL'
        ? 'border-red-200'
        : asset.risk_category === 'HIGH'
        ? 'border-amber-200'
        : 'border-slate-200'
    } p-5 shadow-xs hover:border-slate-300`}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        ...
        <div className="flex items-center gap-3 self-end sm:self-center shrink-0">
          <div className="text-right hidden sm:block">
            <div className="text-[10px] font-mono font-bold text-slate-400 uppercase">COMPOSITE RISK</div>
            <div className="text-xs font-mono text-slate-600 font-semibold">IEEE + Weather</div>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 my-4">
        <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200 text-center">
          ...
          <div className={`font-mono text-sm font-bold ${asset.oil_temp_c > 100 ? 'text-red-600' : 'text-slate-900'}`}>
        ...
        <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200 text-center">
          ...
          <div className={`font-mono text-sm font-bold ${asset.vibration_mms > 6 ? 'text-amber-600' : 'text-slate-900'}`}>
      ...
      {asset.risk_factors && asset.risk_factors.length > 0 && (
        <div className="mb-4">
          <div className="text-[11px] font-mono font-bold text-slate-400 mb-1.5 uppercase tracking-wider">
      ...
      {Object.keys(dga).length > 0 && (
        <div className="border-t border-slate-100 pt-3 mb-4">
          <button
            ...
            className="w-full flex items-center justify-between text-xs font-mono font-bold text-blue-600 hover:text-blue-700 py-1 transition-colors cursor-pointer"
          >
          ...
          {isExpanded && (
            <div className="mt-3 p-3 rounded-lg bg-slate-50 border border-slate-200 space-y-1.5 text-xs">
              <div className="grid grid-cols-5 gap-2 text-center border-b border-slate-200 pb-1.5 font-mono text-[10px] font-bold text-slate-400 uppercase">
                <div>Gas</div>
                <div>Formula</div>
              ...
              {[
                ...
                return (
                  <div key={idx} className="grid grid-cols-5 gap-2 text-center py-0.5 font-mono items-center">
                    <span className="text-slate-800 font-sans font-medium">{g.name}</span>
                    <span className="text-slate-400">{g.formula}</span>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-3 border-t border-slate-100 text-xs">
        ...
        <button
          ...
          className={`w-full sm:w-auto flex items-center justify-center gap-2 px-3.5 py-2 rounded-lg text-xs font-semibold tracking-wide transition-colors cursor-pointer ${
            isHighUrgency
