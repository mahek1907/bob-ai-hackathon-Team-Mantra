...
export default function FleetRiskChart({ assets = [], onSelectAsset }) {
  ...
  return (
    <div className="bg-dark-800 rounded-xl border border-slate-700/60 glass-panel-hover animate-fade-in-up shadow-panel overflow-hidden">
      
      {/* Chart Header */}
      <div className="p-5 border-b border-slate-700/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-dark-900/50">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Real-time multi-variable scoring across IEEE C57.104 gas ratios, weather stress, and topological criticality
          </p>
        </div>
      ...
      <div className="p-6 relative">
        ...
        {/* The Bars Grid */}
        <div className="grid grid-cols-4 gap-6 sm:gap-12 h-64 items-end border-b border-slate-700/60 pb-2">
          {assets.map((asset) => {
              ...
              >
                ...
                <span className={`text-xs font-mono font-bold mb-2 px-2 py-0.5 rounded transition-all ${
                  isSelected 
                    ...
                    : 'bg-dark-700 text-slate-200 group-hover:bg-dark-600'
                }`}>
                ...
                <div 
                  className={`w-full max-w-[72px] rounded-t-lg transition-all duration-300 ${getBarColor(asset.composite_risk_score)} ${
                    isSelected ? 'ring-2 ring-offset-2 ring-white/10 shadow-md' : 'opacity-90 group-hover:opacity-100'
                  }`}
                ...
                <div className="text-center mt-3">
                  <div className={`font-mono font-bold text-xs sm:text-sm ${isSelected ? 'text-blue-400' : 'text-slate-100'}`}>
                    {asset.asset_id}
                  </div>
      ...
      {activeAsset && (
        <div className="p-5 bg-dark-900 border-t border-slate-700/60">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            
            {/* Left: Info */}
            <div className="space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-mono font-bold text-base text-slate-100">{activeAsset.asset_id}</span>
                <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded border uppercase ${getBadgeColor(activeAsset.risk_category)}`}>
              ...
              <div className="text-xs text-slate-300 font-medium">
                Diagnosis: <strong className="text-slate-100">{activeAsset.risk_factors?.[0] || 'Nominal operating parameters'}</strong>
              </div>
            </div>

            {/* Middle: 4 Quick Telemetry Badges */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              <div className="p-2 rounded-lg bg-dark-800 border border-slate-700/60">
                <span className="text-[10px] text-slate-500 block font-mono">OIL TEMP</span>
                <span className={`font-mono font-bold ${activeAsset.oil_temp_c > 100 ? 'text-red-400' : 'text-slate-100'}`}>
                  {activeAsset.oil_temp_c}°C
                </span>
              </div>
              <div className="p-2 rounded-lg bg-dark-800 border border-slate-700/60">
                <span className="text-[10px] text-slate-500 block font-mono">VIBRATION</span>
                <span className={`font-mono font-bold ${activeAsset.vibration_mms > 6 ? 'text-amber-400' : 'text-slate-100'}`}>
                  {activeAsset.vibration_mms} mm/s
                </span>
              </div>
              <div className="p-2 rounded-lg bg-dark-800 border border-slate-700/60">
                <span className="text-[10px] text-slate-500 block font-mono">GRID LOAD</span>
                <span className="font-mono font-bold text-slate-100">
                  {activeAsset.load_pct}%
                </span>
              </div>
              <div className="p-2 rounded-lg bg-dark-800 border border-slate-700/60">
                <span className="text-[10px] text-slate-500 block font-mono">POPULATION</span>
                <span className="font-mono font-bold text-slate-100">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs font-semibold shadow-panel transition-colors cursor-pointer shrink-0"
            >
