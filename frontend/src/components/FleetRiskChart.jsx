              ...
              >
                ...
                <div 
                  className={`w-full max-w-[72px] rounded-t-lg transition-all duration-300 ${getBarColor(asset.composite_risk_score)} ${
                    isSelected ? 'ring-2 ring-offset-2 ring-slate-900 shadow-md' : 'opacity-90 group-hover:opacity-100'
                  }`}
                ...
                <div className="text-center mt-3">
                  <div className={`font-mono font-bold text-xs sm:text-sm ${isSelected ? 'text-blue-600' : 'text-slate-800'}`}>
                    {asset.asset_id}
                  </div>
                  <div className="text-[11px] text-slate-400 font-medium truncate max-w-[90px] sm:max-w-[120px]">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="space-y-1">
              ...
              <div className="text-xs text-slate-600 font-medium">
                Diagnosis: <strong className="text-slate-800">{activeAsset.risk_factors?.[0] || 'Nominal operating parameters'}</strong>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              <div className="p-2 rounded-lg bg-white border border-slate-200">
                <span className="text-[10px] text-slate-400 block font-mono">OIL TEMP</span>
                <span className={`font-mono font-bold ${activeAsset.oil_temp_c > 100 ? 'text-red-600' : 'text-slate-800'}`}>
                  {activeAsset.oil_temp_c}°C
                </span>
              </div>
              <div className="p-2 rounded-lg bg-white border border-slate-200">
                <span className="text-[10px] text-slate-400 block font-mono">VIBRATION</span>
                <span className={`font-mono font-bold ${activeAsset.vibration_mms > 6 ? 'text-amber-600' : 'text-slate-800'}`}>
                  {activeAsset.vibration_mms} mm/s
                </span>
              </div>
              <div className="p-2 rounded-lg bg-white border border-slate-200">
                <span className="text-[10px] text-slate-400 block font-mono">GRID LOAD</span>
                <span className="font-mono font-bold text-slate-800">
                  {activeAsset.load_pct}%
                </span>
              </div>
              <div className="p-2 rounded-lg bg-white border border-slate-200">
                <span className="text-[10px] text-slate-400 block font-mono">POPULATION</span>
                <span className="font-mono font-bold text-slate-800">
