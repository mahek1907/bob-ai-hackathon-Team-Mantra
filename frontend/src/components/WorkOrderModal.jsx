...
export default function WorkOrderModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-3xl rounded-2xl border border-slate-200 bg-white shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in-95">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Target: <span className="font-mono font-bold text-slate-900">{asset.asset_id}</span> ({asset.model}) at <span className="font-bold text-slate-700">{asset.substation_name}</span>
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-600 font-medium">
                    Serves <strong className="text-slate-900 font-bold">{(asset.customers_served || 0).toLocaleString()} customers</strong>
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={handleCopy}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 shadow-xs transition-colors cursor-pointer"
                  >
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                ...
                {signOffResult ? (
                  <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-900 space-y-1">
                    ...
                    <div className="text-xs font-mono space-y-0.5 text-emerald-800">
                      <div>Dispatch Reference ID: <span className="font-bold text-slate-900">{signOffResult.dispatch_id}</span></div>
                      <div>Authorized Operator: <span className="font-bold text-slate-900">{signOffResult.operator_name} ({signOffResult.operator_id})</span></div>
                  ...
                  <form onSubmit={handleSignOff} className="space-y-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          required
                          className="w-full px-3 py-2 text-xs rounded-lg bg-white border border-slate-300 text-slate-900 font-mono focus:outline-none focus:border-blue-500 shadow-xs"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-slate-600 mb-1">
                    ...
                    <label className="flex items-start gap-2.5 text-xs text-slate-700 cursor-pointer pt-1 font-medium">
                      <input
                        type="checkbox"
                        checked={confirmed}
